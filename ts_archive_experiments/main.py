...
if __name__ == '__main__':
    ...
    # QUANT hyperparams
    parser.add_argument('--classifier', type=str, default="XRF", help='Classifier type for Quant: XRF or RF')
    ...
    data = load(config) # note that this adds some other config entries as well
    ...
    tracker = OfflineEmissionsTracker(measure_power_secs=args.measure_power_secs, log_level='error', country_iso_code="DEU", output_dir=config['output_dir'])
    tracker.start()
    if config['use_pretrained']:
        if config['use_pretrained'].endswith('.csv'): # check for path where the original model was stored (given in mlflow experiments summary)
            ...
            crit = (results['params.model'] == config['model']) & (results['params.dataset'] == config['dataset']) & (results['params.fold'] == config['fold'])
    ...
    tracker = OfflineEmissionsTracker(measure_power_secs=args.measure_power_secs, log_level='warning', country_iso_code="DEU", output_dir=config['output_dir'])
    tracker.start()
    results, _ = evaluator()
