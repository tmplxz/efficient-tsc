...
class ConvTran(nn.Module):
    def __init__(self, config):
        ...
        if config['length'] > 128:
            m = 2 ** int(math.log2(config['length'] // 128))
        else:
            m = 1
        config['length'] = config['length'] // m
        # Embedding Layer -----------------------------------------------------------
        self.embed_layer = nn.Sequential(nn.Conv2d(1, emb_size*4, kernel_size=[1, 15], padding='same'),
                                         nn.BatchNorm2d(emb_size*4),
                                         nn.GELU())

        self.embed_layer2 = nn.Sequential(nn.Conv2d(emb_size*4, emb_size, kernel_size=[config['n_channels'], 1], padding='valid'),
                                          nn.BatchNorm2d(emb_size),
                                          nn.GELU())

        self.maxpool = nn.MaxPool2d(kernel_size=(1, m), stride=(1, m))
        self.Fix_Position = tAPE(emb_size, dropout=config['dropout'], max_len=config['length'])
        self.attention_layer = Attention_Rel_Scl(emb_size, num_heads, config['length'], config['dropout'])
        ...
        self.out = nn.Linear(emb_size, config['n_labels'])
